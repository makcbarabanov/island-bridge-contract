...
4. **Отчёт Максу:** если в шаге 3 был **реальный** пуш в `island-bridge-contract`, в конце ответа агент **одной строкой** пишет, что ушло (короткий хеш + тема коммита). README открывать не нужно — достаточно этой строки в чате или журнала на GitHub (ниже).

### У Макса не открыт contract: как видеть, что контекст для агентов обновился

Репозиторий нужен **Бриджу, Форжу и любым агентам** как общий канон (API, БД, runbook). Максу **не обязательно** читать `README.md` внутри contract.

- **Журнал коммитов** (всё на одной странице, дата + сообщение):  
  https://github.com/makcbarabanov/island-bridge-contract/commits/main  
  Любой новый коммит = «контекст на GitHub сдвинулся»; по сообщению видно, API это или косметика в README.
- **Watch на репо** (GitHub → **Watch** → **Custom**): можно включить только уведомления о **commits** / activity — тогда изменения приходят почтой или в веб, без захода в файлы.
- **Чат с агентом:** после пуша contract — явная строка в ответе (см. пункт 4 выше).
