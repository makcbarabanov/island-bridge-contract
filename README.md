...
### Команды Макса: `island`, `bridge`, `контракт`

Термины фиксируем так:

- **`island`** — текущий репозиторий веб-приложения ОСТРОВ (`~/Apps/island`).
- **`bridge`** — сервер/репозиторий бота Bloom (`23.172.217.180`), отдельный контур.
- **`контракт`** — репозиторий `island-bridge-contract` (этот репо).

Смысл команд:

- **«запуш контракт»** → пушить только `island-bridge-contract`.
- **«запуш island»** → пушить только `island`.
- **«запуш оба»** → пушить `island` + `контракт`.
- **`bridge` не входит** в команды выше автоматически.

Отчёт после пуша: две строки в явном виде — `island: ...`, `контракт: ...` (или «без изменений»).

### Маркировка авторства изменений

Чтобы не терялось, кто обновил контракт:

- В каждом изменении markdown указывать подпись автора в тексте блока/заметки: **«Автор: Форж»** или **«Автор: Бридж»**.
- В сообщении коммита использовать префикс: `forge:` или `bridge:` (для быстрого фильтра в истории).
- Если правка совместная, допускается `Автор: Форж, Бридж`.
